...
public static class DemoDataSeeder
{
    ...
    private sealed class DemoSeedRun
    {
        ...
        public async Task RunAsync()
        {
            ...
            //
            // Photography follows DESIGN-BRIEF.md §8: product shots are real, openly-licensed
            // stock photos cut out onto the card colour; the disclosed-defect shots are a pool
            // of six un-retouched "phone photos" (defect-01..06.png) reused across listings and
            // deliberately not matched to the product. Both sets, and every source credit, are
            // generated by tools/demo-images/generate-demo-images.ps1 from tools/demo-images/manifest.json.
            ...
            var toaster = await CreateListingAsync(kitchenCo, adminId, new ListingSpec(
                ...
                WarrantyType.ManufacturerWarranty, 24, 14.000m, null, 4, "toaster.png", "defect-01.png"));
            var blender = await CreateListingAsync(kitchenCo, adminId, new ListingSpec(
                ...
                WarrantyType.ManufacturerWarranty, 12, 24.000m, null, 4, "blender.png", "defect-02.png"));
            ...
            var uprightVacuum = await CreateListingAsync(kitchenCo, adminId, new ListingSpec(
                "home-cleaning", ConditionChoice.ExDisplay, "Upright Vacuum Cleaner",
                ...
                WarrantyType.ShopWarranty, 3, 45.000m, 79.000m, 2, "upright-vacuum.png", "defect-03.png"));

            var handheldVacuum = await CreateListingAsync(petraTools, adminId, new ListingSpec(
                "home-cleaning", ConditionChoice.BoxOpenedOrDamaged, "Handheld Vacuum",
                "Unused handheld vacuums from the winter range. Several outer cartons were opened and taped shut in the warehouse — photo included. Only a few left.",
                WarrantyType.None, null, 22.000m, null, 4, "handheld-vacuum.png", "defect-06.png"));
            await CreateListingAsync(petraTools, adminId, new ListingSpec(
                ...
                WarrantyType.ManufacturerWarranty, 24, 38.000m, null, 5, "cordless-drill.png", "defect-04.png"));
            await CreateListingAsync(petraTools, adminId, new ListingSpec(
            ...
            await CreateListingAsync(petraTools, adminId, new ListingSpec(
                ...
                WarrantyType.None, null, 19.000m, null, 3, "angle-grinder.png", "defect-05.png"));
            // Left in the moderation queue for the admin walkthrough — not approved.
            await CreateListingAsync(petraTools, adminId, new ListingSpec(
                "power-tools", ConditionChoice.Sealed, "Precision Screwdriver Set",
                "20-piece precision set from the overstock run. Sealed.",
                WarrantyType.None, null, 6.500m, null, 5, "screwdriver-set.png", null), approve: false);
            await CreateListingAsync(petraTools, adminId, new ListingSpec(
            ...
            await CreateDraftListingAsync(unsubscribed, new ListingSpec(
                "small-kitchen-appliances", ConditionChoice.BoxOpenedOrDamaged, "Espresso Machine",
                "Overstock unit; the retail box was opened for a counter display and re-taped. Ready to publish once the shop's subscription is active.",
                WarrantyType.ManufacturerWarranty, 12, 60.000m, null, 2, "kettle.png", "defect-01.png"));
        ...
        /// </summary>
        private async Task<DemoListing> CreateListingAsync(
            DemoMerchant merchant, string adminId, ListingSpec spec, bool approve = true)
        {
            var outcome = OkValue(
            ...
            // One listing is left in the moderation queue on purpose, so the admin walkthrough
            // has a real listing to approve. Everything else is approved here, exactly as a
            // moderator would.
            if (approve)
            {
                Ok(
    ...
    /// <summary>
    /// Media fixtures for the Development-only demo seed. Product and defect photography is
    /// real, openly-licensed stock (CC0 / CC BY / CC BY-SA from Wikimedia Commons and
    /// StockSnap; Pexels-licensed defect shots) — every file is credited in
    /// <c>docs/CREDITS.md</c>. The committed PNGs are produced by
    /// <c>tools/demo-images/generate-demo-images.ps1</c> from the sources and attribution list
    /// in <c>tools/demo-images/manifest.json</c>: product shots are cut out (remove.bg) and
    /// composited onto the card colour, defect shots keep their real background and are only
    /// square-cropped (DESIGN-BRIEF.md §8). Each file lives under
