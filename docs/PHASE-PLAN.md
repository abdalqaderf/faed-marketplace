...
rebuild · demo readiness) were planned in `docs/DESIGN-BRIEF.md` and are complete; they are
back-filled below as Do / Acceptance / Verify sections written from what the code does.
**Phase 14** closes the branch.
...
## Phase 11 — Design system (complete)

Planned in `docs/DESIGN-BRIEF.md`, executed as tokens and mockups, no application code.

**Do**
- Add the `--faed-*` tokens to `wwwroot/css/faed.css`: `--faed-ink`, `--faed-paper`,
  `--faed-card-bg`, `--faed-rust`, `--faed-success`, `--faed-danger`, `--faed-radius` (2px),
  and the three font stacks (Archivo, Archivo Narrow, IBM Plex Sans), loaded from Google Fonts
  in `_Layout.cshtml`
- Mock the five public screens (Home, Shop, Listing detail, Reserve, Merchant storefront)
  against those tokens
- Fix the condition-indicator mechanic: one hue, four fill levels, a photo-corner stamp —
  never a second colour on the same screen

**Acceptance**
- Every token, the ink/rust colour rule and the condition-stamp treatment are locked in
  `docs/DESIGN-BRIEF.md` §5, with a decision log recording what changed from the original
  brief (accent colour, radius, the retired fashion-era tokens)
- All five public screens are specified in writing, not left to interpretation in Phase 12

**Verify:** `dotnet build Faed.slnx` — no application code changed in this phase

---

## Phase 12 — Public surface rebuilt (complete)

**Do**
- Rebuild the five public screens to the fx- component set: `_Layout.cshtml`'s
  `isPublicSurface` branch (wordmark, three-link nav, one-line footer), `Views/Home/Index`,
  `Views/Shared/_ShopBrowse` (Shop), `Views/Listing/Details` (gallery, condition stamp,
  disclosed-condition section), `Views/Store/Index` (storefront), `Areas/Buyer/Views/Reservation/Index`
  (Reserve — routed under `Areas/Buyer` but rendered as a public page per
  `docs/DESIGN-BRIEF.md` §5.4a)
- Token pass only on the Merchant and Admin workspaces and on Identity (`faed-auth-layout`) —
  colours, spacing and type, not a rebuild
- Add an empty state to every list screen that can be empty: `faed-empty` /
  `fx-empty` blocks appear across the admin, merchant and buyer areas (listing moderation,
  merchant verification queue, reviews, subscriptions, orders, analytics, listings, reservation)
- Identity avatar fixed to the single square-ink shape (`docs/DESIGN-BRIEF.md` §5.9) everywhere
  a merchant identity renders

**Acceptance**
- The condition stamp renders in exactly one place per screen — the corner of the primary
  photo — on cards and on the listing detail page alike
- The New Seller badge replaces the response-rate line below five orders, with no percentage
  shown alongside it, in the identical layout position
- No advanced-filter drawer; the Shop and storefront filter rows match
  `docs/DESIGN-BRIEF.md` §5.7/§5.8
- Every list screen shows a named empty state instead of a blank table

**Verify:** `dotnet build Faed.slnx && dotnet test`, then a manual click-through of all five
public screens

---

## Phase 13 — Demo readiness (complete)

**Do**
- Rewrite the product photography in `DemoDataSeeder`: 12 product cut-outs and 6 reused defect
  photos, generated by `tools/demo-images/generate-demo-images.ps1` from CC-licensed sources,
  attributed in `docs/CREDITS.md`
- Write `docs/DEMO-SCRIPT.md`: a screen-by-screen defence walkthrough naming the seeded
  accounts, listings and order references
- Walk all three roles end to end by hand against the seeded data

**Acceptance**
- The demo seed produces a browsable catalogue with real photography, no placeholder art
- Every photo's licence is recorded in `docs/CREDITS.md` before it ships
- `docs/DEMO-SCRIPT.md` names an account, a URL and an expected result for each step of the
  walkthrough

**Verify:** `Faed__DemoSeed__Enabled=true` seed run, then the walkthrough in
`docs/DEMO-SCRIPT.md`

---

## Phase 14 — Outcome (in progress)

Findings, deletion list and rehearsal log: `docs/AUDIT-PHASE-14.md`. Landed so far, one commit
each: the Step 5 keep-list amended to include `DESIGN-BRIEF.md`, `DEMO-SCRIPT.md`,
`CREDITS.md` and `tools/demo-images/`; eight general UI rules moved out of `plan.md` into
`CLAUDE.md` and `DESIGN-BRIEF.md` before it was deleted; the two-sided copy removed from
`_Layout.cshtml`; the deleted `Brand` entity dropped from admin catalogue copy; six dead paths
removed in one commit (`plan.md`, `design-previews/`, `Claude outputs/`,
`Properties/PublishProfiles/`, `Properties/serviceDependencies*.json`,
`wwwroot/js/listing-detail.js`); `docs/04-DOMAIN-MODEL.md` regenerated from the entities with
one adopted count (17 aggregate roots, 21 mapped EF types); `DEPLOYMENT.md` rewritten to the
real Brevo/R2 infrastructure and moved to `docs/`; `README.md` brought current with a Demo
accounts block; the Phase 9 "Actually collected" no-show recovery implemented with two tests;
and the demo seed photography shrunk from ~19 MB to ~1.4 MB. Step 9's rehearsal and
`docs/REHEARSAL-CHECKLIST.md` follow once the fix list is exhausted.
...
| `docs/DESIGN-BRIEF.md` | holds locked visual decisions — the `--faed-*` tokens, the ink/rust colour rule, the condition-stamp treatment — that no other doc restates |
...
the list, do not delete the file.
