...
/// <summary>
/// The B2B deal routes through the real MVC pipeline (tasks/TASK-008-B2B-DEALS.md;
/// docs/16-PERMISSIONS-MATRIX.md "View unrelated B2B negotiation — ❌"). Authorization and
/// participation are asserted at the HTTP surface, not only at the service layer.
/// </summary>
[Collection(FaedWebCollection.Name)]
public sealed class B2BDealHttpTests(FaedWebApplicationFactory factory)
{
    ...
    [SkippableFact]
    public async Task DealsQueue_Anonymous_IsChallenged()
    {
        Skip.IfNot(factory.DatabaseReady, "SQL Server not reachable.");
        var response = await Anonymous().GetAsync("/Merchant/Deals");
        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    [SkippableFact]
    public async Task DealsQueue_ForAUserWithoutAnApprovedMerchantProfile_IsForbidden()
    {
        Skip.IfNot(factory.DatabaseReady, "SQL Server not reachable.");
        var response = await As(Guid.NewGuid().ToString()).GetAsync("/Merchant/Deals");
        Assert.Equal(HttpStatusCode.Forbidden, response.StatusCode);
    }

    [SkippableFact]
    public async Task DealsQueue_ForAnApprovedMerchantWithAdminRole_IsForbiddenAtTheAuthorizationBoundary()
    {
        Skip.IfNot(factory.DatabaseReady, "SQL Server not reachable.");
        await using var scope = new Scope(factory);
        var (adminMerchantUserId, _) = await scope.CreateApprovedMerchantAsync();

        var response = await As(adminMerchantUserId, FaedRoles.Admin).GetAsync("/Merchant/Deals");

        Assert.Equal(HttpStatusCode.Forbidden, response.StatusCode);
    }

    [SkippableFact]
    public async Task DealPages_RenderForAParticipant_ButAnUnrelatedMerchantGets404OnTheDetail()
    {
        Skip.IfNot(factory.DatabaseReady, "SQL Server not reachable.");
        await using var scope = new Scope(factory);
        ...
        var accept = await scope.Deals.AcceptOfferAsync(
            sellerUserId, start.Value, new AcceptOfferInput(B2BFulfillmentType.Pickup));
        Assert.True(accept.Succeeded, accept.Error);

        Assert.Equal(HttpStatusCode.OK, (await As(sellerUserId).GetAsync("/Merchant/Deals")).StatusCode);
        Assert.Equal(HttpStatusCode.OK,
            (await As(sellerUserId).GetAsync($"/Merchant/Deals/Details/{accept.Value}")).StatusCode);
        Assert.Equal(HttpStatusCode.OK,
            (await As(buyerUserId).GetAsync($"/Merchant/Deals/Details/{accept.Value}")).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound,
            (await As(strangerUserId).GetAsync($"/Merchant/Deals/Details/{accept.Value}")).StatusCode);
    }
